    ...
    for row_num, row_cells in enumerate(rows_iter, start=2):
        ...
        if _is_empty_row(row_cells_list):
            ...
            if consecutive_empty >= empty_limit:
                logger.info(f"[{sheet_title}] {empty_limit} consecutive empty rows at row {row_num} — stopping early.")
                return
            continue
        consecutive_empty = 0
...
def _validate_url(url: str, excel_row: int, col_name: str, report: "ImportReport") -> Optional[str]:
    if not _URL_RE.match(url):
        report.add("url_skipped", {"Row": excel_row, "Column": col_name, "URL": url, "Reason": "Malformed URL"})
        return None
    return url
...
def _get_or_create_resource(url: str, resource_type: str, comment: Optional[str], cache: Dict[str, ExternalResource], session: Session) -> ExternalResource:
    if url in cache: return cache[url]
    existing = session.exec(select(ExternalResource).where(ExternalResource.url == url)).first()
    if existing:
        cache[url] = existing
        return existing
...
def _get_or_create_text_type(session: Session, name: Optional[str], cache: Dict[str, TextType]) -> Optional[int]:
    if not name: return None
    name = name.strip()
    if name in cache: return cache[name].id
...
def _get_or_create_image_type(session: Session, name: Optional[str], cache: Dict[str, ImageType]) -> Optional[int]:
    if not name: return None
    name = name.strip()
    if name in cache: return cache[name].id
...
def _get_or_create_ms_identifier(session: Session, title: Optional[str], bhl: Optional[str], cache: Dict[str, ManuscriptIdentifier]) -> Optional[int]:
    ...
    if key in cache: return cache[key].id
    existing = session.exec(
        select(ManuscriptIdentifier).where(ManuscriptIdentifier.title == title, ManuscriptIdentifier.bhl_number == bhl)
    ).first()
    if existing:
        cache[key] = existing
        return existing.id
...
def _add_manuscript_resource(
    ...
    cache: Dict[str, ExternalResource], session: Session, report: "ImportReport", stats: Dict[str, int]
) -> None:
    ...
    exists = session.exec(
        select(ManuscriptExternalResource).where(
            ManuscriptExternalResource.manuscript_id == ms.id, ManuscriptExternalResource.resource_id == resource.id
        )
    ).first()
    if not exists:
        session.add(ManuscriptExternalResource(manuscript_id=ms.id, resource_id=resource.id))
...
def _cell_inspection_status(cell: Cell) -> str:
    try:
        fill = cell.fill
        if fill is None or fill.fgColor is None: return "unknown"
        color = fill.fgColor
        if color.type != "rgb": return "unknown"
        argb = color.rgb
        if argb in ("00000000", "FF000000", "FFFFFFFF"): return "unknown"
        r, g, b = int(argb[2:4], 16), int(argb[4:6], 16), int(argb[6:8], 16)
        if r < 100 and g > 150 and b < 100: return "direct"
        if r > 200 and 100 <= g <= 200 and b < 100: return "uncertain"
        if r > 200 and g < 100 and b < 100: return "indirect"
        return "unknown"
    except Exception:
        return "unknown"
...
def row_to_cell_dict(row_cells_list: List[Cell], headers: List[Optional[str]]) -> Dict[str, Cell]:
...
def _read_headers(ws: Worksheet) -> Tuple[List[Optional[str]], Any]:
    ...
    headers = [str(c.value).strip() if c.value is not None else None for c in header_row]
    logger.info(f"[Headers] {headers}")
    return headers, rows_iter


class ImportReport:
    CATEGORIES = ["url_skipped", "relation_inconsistency", "invalid_format", "critical_error"]

    def __init__(self) -> None:
        self.categories: Dict[str, List[Dict[str, Any]]] = {c: [] for c in self.CATEGORIES}
...
def import_texts(session: Session, wb, report: ImportReport) -> Dict[str, Text]:
    ...
    for excel_row, row_cells_list in _iter_data_rows(rows_iter, "Corpus hagio"):
        ...
        # New TextType assignment
        text_type_id = _get_or_create_text_type(session, cval(row, "Mainly prose or verse"), text_type_cache)
        ...
        text = Text(
            ...
            checked_leg=cyesno(row, "Check Narrative Sources"),
...
def import_manuscripts(session: Session, wb, text_cache: Dict[str, "Text"], report: ImportReport) -> Tuple[Dict[int, Manuscript], Dict[str, Manuscript]]:
    ws = wb["Manuscripts"]
    headers, rows_iter = _read_headers(ws)
    if not headers:
    ...
    stats: Dict[str, int] = {"manuscripts_inserted": 0, "manuscripts_skipped": 0, "urls_imported": 0, "urls_skipped": 0, "images_created": 0}
    ...
    for batch in _chunked(_iter_data_rows(rows_iter, "Manuscripts"), 500):
        try:
            for excel_row, row_cells_list in batch:
                ...
                uid_raw, uid_ok = parse_int(row["Unique ID"].value if "Unique ID" in row else None)
                if not uid_ok:
                    report.add("invalid_format", {"Row": excel_row, "Column": "Unique ID", "Value": row["Unique ID"].value if "Unique ID" in row else None, "Reason": "Could not parse as integer"})
                ...
                if uid_raw is not None and uid_raw in ms_cache:
                    ...
                    prov_inst_id = _get_or_create_institution(session, cval(row, "Provenance institution"), coll_loc_id, inst_cache)
                ...
                if img_cell is not None:
                    ...
                    if url:
                        ...
                        if url:
                            itype_str = _infer_image_type(cval(row, _COL_MS_IMAGE_AVAIL))
                            itype_id = _get_or_create_image_type(session, itype_str, image_type_cache)
                            exists = session.exec(select(Image).where(Image.manuscript_id == ms.id, Image.url == url)).first()
                            if not exists:
                                session.add(Image(url=url, image_type_id=itype_id, comment=comment, manuscript_id=ms.id))
    ...
    for src_uid, tgt_uid, rel_type, cert, notes, col, row_num in relation_tasks:
        ...
        session.add(
            ManuscriptRelation(
                source_manuscript_id=src_ms.id, target_manuscript_id=tgt_ms.id, relation_type=rel_type,
                certainty=cert, notes=notes, source_reference=f"Excel import — Manuscripts tab (row {row_num}, column '{col}')"
            )
        )
        rel_created += 1

    session.commit()
    logger.info(f"[Tab 1] Done — {stats['manuscripts_inserted']} inserted, {rel_created} relations created.")
    return ms_cache, ms_coll_cache


def import_editions(session: Session, wb, text_cache: Dict[str, "Text"], ms_cache: Dict[int, Manuscript], ms_coll_cache: Dict[str, Manuscript], report: ImportReport) -> None:
    ws = wb["Editions"]
    headers, rows_iter = _read_headers(ws)
    if not headers:
        return
    ...
    resource_cache: Dict[str, ExternalResource] = {}
    stats: Dict[str, int] = {"editions_inserted": 0, "ms_links_created": 0, "urls_imported": 0, "urls_skipped": 0}
    ...
    for batch in _chunked(_iter_data_rows(rows_iter, "Editions"), 500):
        try:
            for excel_row, row_cells_list in batch:
                ...
                if not bhl and not ed_id: continue
                ...
                edition_ref_parts = [
                    ...
                    if idx < len(row_cells_list) and clean_value(row_cells_list[idx].value)
                ]
                ...
                edition = Edition(
                    bhl_number=bhl, title=cval(row, "Title"), edition_identifier=ed_id,
                    edition_reference_per_text=cval(row, "Ed. reference per individual text"),
                    checked_leg=None, checked_dg=cyesno(row, "DG"), checked_naso=cyesno(row, "NASO"),
                    checked_ed_sec=cyesno(row, "ED/SEC"), unique_id_numeric=cint(row, _COL_ED_UID_NUM),
                    unique_id_descriptive=cval(row, _COL_ED_UID_DESC), year_of_publication=cint(row, _COL_ED_YEAR),
                    bibliographic_reference=cval(row, _COL_ED_BIBREF), page_range=cval(row, _COL_ED_PAGES),
                    is_reprint=cyesno(row, "Reprint?"), reprint_identically_typeset=cyesno(row, "If reprint, identically typeset?"),
                    reprint_newly_typeset=cyesno(row, "If reprint, newly typeset?"), reprint_of=cval(row, _COL_ED_REPRINT_OF),
                    has_scan=has_scan, has_transcription=cyesno(row, _COL_ED_TRANSCRIBED), transcription_notes=cval(row, "Our transcribed ed.?"),
                    collated=cyesno(row, _COL_ED_COLLATED), edition_refs=", ".join(edition_ref_parts) or None,
                    notes=cval(row, "Notes"), text_id=text_obj.id if text_obj else None,
                )
                session.add(edition)
                session.flush()
                ...
                if scan_url:
                    ...
                    if scan_url:
                        resource = _get_or_create_resource(scan_url, "scan", scan_comment, resource_cache, session)
                        exists = session.exec(select(EditionExternalResource).where(EditionExternalResource.edition_id == edition.id, EditionExternalResource.resource_id == resource.id)).first()
                        if not exists:
                            session.add(EditionExternalResource(edition_id=edition.id, resource_id=resource.id))
                            stats["urls_imported"] += 1
                elif scan_cell and scan_cell.value:
                    stats["urls_skipped"] += 1

                for idx in _EDITION_MS_COL_INDICES:
                    if idx >= len(row_cells_list): continue
                    ms_cell = row_cells_list[idx]
                    ms_val = str(ms_cell.value).strip() if ms_cell.value else ""
                    if not ms_val or ms_val.upper() == "N/A": continue
