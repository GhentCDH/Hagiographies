#!/usr/bin/env bash
# Backup the PRD database
#
# .env holds the remote connection strings:
#   PG_DATABASE_URL_PRD  -> PRD
set -euo pipefail
...
DUMP_DIR="$(pwd)/data"
FILE="prd_backup_$(date -Iseconds).dump"
DUMP_FILE="/dump/${FILE}"
...
docker run --rm \
    ...
    --dbname="${PG_DATABASE_URL_PRD}" \
    --file="${DUMP_FILE}"

echo "==> Done. PRD now backed up (saved to data/${FILE})."
