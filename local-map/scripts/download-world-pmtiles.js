/**
 * Download Protomaps basemap PMTiles files
 *
 ...
 * file (~130 GB at full zoom). This script extracts lightweight subsets.
 *
 ...
 *
 * Usage:
 *   node scripts/download-world-pmtiles.js [--dry-run] [--maxzoom=N]
 *   node scripts/download-world-pmtiles.js --bbox=S,W,N,E --output=name.pmtiles [--maxzoom=N]
 */
...
const bboxFlag = args.find((a) => a.startsWith("--bbox="));
const bbox = bboxFlag ? bboxFlag.split("=")[1] : null;
const outputFlag = args.find((a) => a.startsWith("--output="));
const outputFile = outputFlag ? outputFlag.split("=")[1] : "world.pmtiles";
...
async function main() {
  ...
  const outPath = join(DATA_DIR, outputFile);
